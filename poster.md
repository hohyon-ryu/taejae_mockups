...
**러셀 액코프(Russell Ackoff)가 1989년 논문 "From Data to Wisdom"에서 체계화한 의사결정 과정 모델**
...
- **데이터**: 가공되지 않은 원시 사실과 수치. 맥락이 없는 단순한 관찰 결과나 측정값.
- **정보**: 맥락이 부여된 데이터. 의미와 목적을 가지고 조직화된 데이터 집합.
- **지식**: 패턴이 인식된 정보. 경험과 학습을 통해 형성된 구조화된 정보 체계.
- **지혜**: 가치와 판단이 더해진 지식. 장기적 관점에서 올바른 결정을 내리는 능력.

이 모델은 *의사결정 과정*을 단계적으로 보여줍니다: 데이터 수집 → 맥락화된 정보 생성 → 패턴 인식을 통한 지식 구축 → 가치 판단을 통한 지혜로운 결정. 각 단계마다 인간과 기술의 역할이 변화하며, 이는 사회 구조와 권력 관계에도 영향을 미칩니다.
